...
local function updateSpinning(deltaTime, center)
    if isSpinning then
        angle = angle + speed * deltaTime
        local newX = center.Position.X + radius * math.cos(angle)
        local newZ = center.Position.Z + radius * math.sin(angle)
        rootPart.CFrame = CFrame.new(newX, rootPart.Position.Y, newZ) * CFrame.Angles(0, angle, 0)
    end
end

local function checkSound(Boss)
    if sound.IsPlaying then
        stopSpinning()
        local successMove = pcall(function()
            rootPart.CFrame = Boss.CFrame * CFrame.new(0, 0, -3)
        end)
    else
        startSpinning()
        local successReset = pcall(function()
            rootPart.CFrame = Boss.CFrame * CFrame.new(33, 0, 0)
        end)
    end
end

local function checkSpiderHitbox()
    local success, center = pcall(function()
        return workspace.BossBattle.Saigomo:WaitForChild("SpiderHitbox")
    end)

    if not success or not center then
        return nil
    end
...
local function runGameLoop(deltaTime)
    local center = checkSpiderHitbox()
    if not center then return end

    local Boss = game.Workspace.BossBattle.Saigomo.HumanoidRootPart
    
    local successUpdate = pcall(function()
        updateSpinning(deltaTime, center)
    end)
...
RunService.Heartbeat:Connect(runGameLoop)
